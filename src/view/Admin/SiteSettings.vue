    ...
    >
      <div class="space-y-4 text-left">
        ...
        <div>
          ...
          <input type="file" accept="image/*" @change="handleImageUpload" class="w-full border rounded-md px-3 py-2" />
        </div>
        ...
        <div v-if="previewImage" class="flex justify-center mt-2">
          <img :src="previewImage" alt="Preview" class="w-24 h-24 object-cover rounded-full border border-gray-300 shadow-sm" />
        </div>
        ...
        <div>
          ...
          <input v-model.number="selectedGallon.price" type="number" step="0.01" class="w-full border rounded-md px-3 py-2" />
        </div>
      </div>
      ...
      <template #actions>
        ...
        <button
          @click="handleSave"
          class="px-4 py-2 rounded-md bg-cyan-600 hover:bg-cyan-700 text-white"
        >
          Save
        </button>
      </template>
